...
abstract class AbstractDatabaseTest extends Specification {
    ...
    def "should save invoices returning sequential id, id should have correct value, findById should return saved invoice"() {
        ...
        ids.forEach({ assert database.get(it).getId() == it })
        ids.forEach({ assert database.get(it).getId() == invoices.get(it as int - 1).getId() })
    }
    ...
    def "should be possible to update the invoice"() {
        ...
        then:
        database.get(id).getId() == invoices.get(0).getId()
    }
